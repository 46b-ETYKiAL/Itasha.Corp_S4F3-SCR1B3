...
- **Workspace shape:** three crates — `scribe-core` (engine: buffer, file I/O, encoding/EOL, config, theme, syntax, search, update logic; **no UI dependency**), `scribe-render` (maps the engine `Theme` onto egui `Visuals` and carries CRT parameters), and `scribe-app` (the binary: window, tabs, find bar, status bar, frameless titlebar). The core/UI seam is kept clean so the rendering shell is replaceable without re-architecting the engine.
